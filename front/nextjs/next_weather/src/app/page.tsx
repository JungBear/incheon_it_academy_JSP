...
// https:/api.openweathermap.org/data/2.5/forecast?q=incheon&appid=c9bbe193d3f3fd65c3e908660e06607c&cnt=56
    ...
    <div className={styles.homeContainer}>
      ...
      <main className={styles.homeMain}>
        ...
        <section className={styles.spaceY4}>
              ...
              <div className={styles.todayInfo}>
                {
                  data?.list.map((item, idx) =>{
                    return(
                      <div className={styles.todayItem} key={idx}>
                        <p>
                          {item.dt_txt}
                        </p>
                        {/* 아이콘 */}
          ...
          <div className={styles.todatSecondBox}>
            <ListBox className="box-style3">
              <p className={styles.todatSecondTitle}>
            ...
            <ListBox className="box-style4">
              <TodayDetail 
                visibility={convertMetersToKilometers(todayData?.visibility?? 10000)} humidity={`${todayData?.main.humidity}%`} windSpeed={convertWindSpeed(todayData?.wind.speed ?? 0.0)} 
                airPressure={`${todayData?.main.pressure}hPa`} sunrise={format(fromUnixTime(data?.city.sunrise ?? 0), "H:mm")} sunset={format(fromUnixTime(data?.city.sunset ?? 0),"H:mm")}/> 
            </ListBox>
          </div>
        </section>
...
// favicon.ico 삭제 -> icon.png 추가
// npm install react-icons --save
// npm install react-query axios classnames
// npm install date-fns@2.30.0 

// axios(fetch를 쉽게 해주는 라이브러리)